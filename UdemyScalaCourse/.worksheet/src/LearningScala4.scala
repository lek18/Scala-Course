...
println(shipMap("Janeway"));$skip(95); 
//println(shipMap("Voyager"))
...
val mylist=(1 to 20).toList;System.out.println("""mylist  : List[Int] = """ + $show(mylist ));$skip(31); val res$0 = 
mylist.filter((x:Int)=>x%3==0);System.out.println("""res0: List[Int] = """ + $show(res$0));$skip(49); 
for(nums <- mylist){
if(nums%3==0) println(nums)}
}
