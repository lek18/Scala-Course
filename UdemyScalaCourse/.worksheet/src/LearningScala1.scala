object LearningScala1 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(126); 
  ...
  val isBest: Boolean = picard == bestCaptain;System.out.println("""isBest  : Boolean = """ + $show(isBest ));$skip(339); 
  ...
  def doublePi(x:Double): Double ={
  val y= x*2
  y
  };System.out.println("""doublePi: (x: Double)Double""");$skip(21); 
  val y=doublePi(pi);System.out.println("""y  : Double = """ + $show(y ));$skip(21); 
  println(f"$y%.3f")}
